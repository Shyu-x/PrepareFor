...
| 维度 | 内容 |
|------|------|
| **Situation (背景)** | 团队希望构建一个完整的 Web 版操作系统，提供类似本地开发环境的体验，实现浏览器内的 IDE 和容器化开发环境 |
| **Task (任务)** | 实现虚拟桌面、窗口管理、Monaco 编辑器集成、终端模拟、Docker 容器管理、多人协作等核心功能 |
| **Action (行动)** | 采用 Next.js 前端架构，NestJS 后端服务，WebContainer 实现 Node.js 运行时，Docker 容器池提供隔离开发环境 |
| **Result (成果)** | 实现了完整的 Web IDE 功能，支持 8 种开发环境容器，多人实时协作，Monaco 编辑器完整集成 |
...
flowchart TB
    subgraph Client["前端 (Next.js)"]
        Desktop["桌面环境<br/>Desktop/WindowManager"]
        IDE["IDE 编辑器<br/>Monaco/xterm.js"]
        Apps["应用生态<br/>Docker/Terminal/Git"]
    end

    subgraph Backend["后端 (NestJS)"]
        Gateway["API Gateway"]
        Terminal["Terminal Gateway<br/>WebSocket"]
        Docker["Docker Service<br/>容器管理"]
        DB["PostgreSQL<br/>数据持久化"]
    end

    subgraph Runtime["运行时"]
        WebContainer["WebContainer<br/>Node.js 运行时"]
        DockerPool["Docker 容器池"]
    end
...
flowchart LR
    subgraph Nginx["Nginx 反向代理"]
        direction TB
        Frontend["前端静态<br/>:11451"]
        BackendAPI["后端API<br/>:18888"]
    end

    subgraph Services["服务层"]
        direction TB
        NextJS["Next.js<br/>前端"]
        NestJS["NestJS<br/>后端"]
    end

    subgraph Data["数据层"]
        direction TB
        Postgres["PostgreSQL<br/>:15433"]
    end

    subgraph Containers["容器池 (DinD)"]
        direction TB
        NodeImg["Node.js<br/>开发环境"]
        PythonImg["Python<br/>开发环境"]
        GoImg["Go<br/>开发环境"]
    end
...
> 本部分内容与第一部分中的项目结构总览相对应，是前端目录的详细展开说明。
